...
class GameOverGameScene: SKScene {
    ...
    private func addCenterButton() {
        ...
        let buttonSize = CGSize(width: 320, height: 120) // Substitua pelos valores desejados
        button.size = buttonSize
        
        // Configurar a posição do botão no meio da tela
        button.position = CGPoint(x: size.width / 3, y: size.height - size.height / 1.16)
        
        

        button.name = "goToMenu"
        addChild(button)
    ...
    private func addPlayAgainButton() {
        ...
        // Configurar a posição do botão no meio da tela
        button.position = CGPoint(x: size.width / 3, y: size.height - size.height / 1.4)
